...
{
    $connection = _getConnection('core_read');
    $sql        = "SELECT id FROM " . _getTableName('zoho_inactive_data') . " WHERE lead_id = ?";
    return $connection->fetchOne($sql, [$data['lead_id']]);
}

function uploadData($datas) {
 $last_id = 0;
    $tableEntity = _getTableName('zoho_inactive_data');
     $connection   = _getWriteConnection();
     $last_id_arr = array();
...
function _refZohoToken() {
    ...
    $query = "select Full_Name,Email,Phone,Lead_Status,Lead_Source,Lead_Type,Main_Category,Product_Category,Lead_Country,Budget_Range,Referrer from Leads where Lead_Status='Lead Inactive'";
