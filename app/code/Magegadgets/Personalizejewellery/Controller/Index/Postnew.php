...
class Postnew extends \Magento\Framework\App\Action\Action
{
	...
	/*echo "<pre>";
		...
		if ($data['type']== 2){
		  ...
		  $details = $data['details'];
		  $budget = $data['budget'];
		  $image = 'personalizejewellery_personalizejewellery/c/a/cancelbeforedispatch.png';
		}
		if ($data['type']== 3){
		  $name = $data['fullname'];
		  $email = $data['email'];
		...
		$model->setData($data);
		 ...
		 /* end of entry in lead */
		 /*entry in Lead @if data['type']==2*/
		 if($data['type']== 2){
            $data_string = '[{"Attribute":"EmailAddress","Value":"'.$email.'"},{"Attribute":"FirstName","Value":"'.$name.'"},{"Attribute":"Notes","Value":"Customizing design"},{"Attribute":"Mobile","Value":"'.$mobile.'"},{"Attribute":"mx_Budget","Value":"'.$budget.'"},{"Attribute":"mx_Details_of_Jewellery","Value":"'.$details.'"},{"Attribute":"mx_Image","Value":"'.$image.'"},{"Attribute":"mx_Product_SKU","Value":"'.$sku.'"}]';
            try
            {
            $curl = curl_init('https://api-in21.leadsquared.com/v2/LeadManagement.svc/Lead.Create?accessKey=u$r8558704a813544073cf33583654e3111&secretKey=42dda614196ea45f10ee0d324bca5097d84ca6dc');
            curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "POST");
		 ...
		 /* end of entry in lead */
		  /*entry in Lead @if data['type']==3*/
		 if($data['type']== 3){
            $data_string = '[{"Attribute":"EmailAddress","Value":"'.$email.'"},{"Attribute":"FirstName","Value":"'.$name.'"},{"Attribute":"Source","Value":"Work With our Designers"},{"Attribute":"Mobile","Value":"'.$mobile.'"},{"Attribute":"mx_Budget","Value":"'.$budget.'"},{"Attribute":"mx_Details_of_Jewellery","Value":"'.$details.'"}]';
            try
            {
            $curl = curl_init('https://api-in21.leadsquared.com/v2/LeadManagement.svc/Lead.Create?accessKey=u$r8558704a813544073cf33583654e3111&secretKey=42dda614196ea45f10ee0d324bca5097d84ca6dc');
            curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "POST");
			...
			try
        {
		   ...
		   $templateId="1407160956956365096";
