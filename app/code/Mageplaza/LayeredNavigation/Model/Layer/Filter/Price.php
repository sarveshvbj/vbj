...
class Price extends AbstractFilter
{
    ...
    {
         $writer = new \Zend\Log\Writer\Stream(BP.'/var/log/pricefilter.log');
        ...
        list($from, $to) = $this->_filterVal = $filter;
          $logger->info("From_current".$from." To_current".$to);
    ...
    protected function _renderRangeLabel($fromPrice, $toPrice, $isLast = false)
    {
        $writer = new \Zend\Log\Writer\Stream(BP.'/var/log/pricefilter.log');
            $logger = new \Zend\Log\Logger();
            $logger->addWriter($writer);
            $logger->info("From_render".$fromPrice." Toprice_render".$toPrice);
