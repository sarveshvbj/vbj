...
use Potato\Compressor\Model\Optimisation\Processor\Finder\RegExp\ScriptTag as RegExpScript;
...
class Image
{
     ...
     * @param RegExpScript $scriptTagFinder
    ...
    public function processLazyLoad(&$html)
    {
        ...
        foreach ($tagList as $tag) {
            ...
            if (array_key_exists('src', $attributes) && strpos($attributes['src'], 'noscript')) {
                continue;
            }
            if (array_key_exists('src', $attributes) && strpos($attributes['src'], '<%')) {
                continue;
            }
            $replaceData[] = [
                'start'   => $tag->getStart(),
     ...
     * @throws \Exception
     */
    public function processFixProductGallery(&$html)
    {
        $replaceData = [];
        ...
        foreach ($tagList as $tag) {
            ...
            if (trim($type) !== 'text/x-magento-init') {
                continue;
            }
            if (strpos($tag->getContent(), '"mage/gallery/gallery"') === FALSE
                || strpos($tag->getContent(), 'data-gallery-role') === FALSE) {
                continue;
            }
            $replaceData[] = [
                'start'   => $tag->getStart(),
                ...
                'content' => $tag->getContentWithUpdatedAttribute(
                    [
                        DataHelper::DATA_ATTRIBUTE_FOR_FAST_INIT => ""
                    ]
                )
            ];
        }
