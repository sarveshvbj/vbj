...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getCustomPrice($id, $sku, $ringsize, $purity, $diamond_quality)
    {
      ...
      $diamond_color = $diamond_quality_values[1];
      $diamond_qual = $diamond_quality_values[0];
    } else {
      $diamond_color = "0";
      $diamond_qual = "0";
    }
     ...
     public function checkValidProduct($id, $sku, $ringsize, $purity, $diamond_quality)
    {
        $result = array();
        if($diamond_quality !="0") { 
      $diamond_quality_values = explode("-", $diamond_quality, 2);
      ...
      $diamond_qual = "0";
        ...
        $connection = $this
            ...
            ->getConnection(\Magento\Framework\App\ResourceConnection::DEFAULT_CONNECTION);
        $customProductTable = $connection->getTableName('custom_config_products');
        $sql = "SELECT * FROM " . $customProductTable . " WHERE id=? AND sku = ? AND size = ? AND purity = ? AND diamond_color = ? AND diamond_quality = ?";
        $product_values = $connection->fetchAll($sql, [$id, $sku, $ringsize, $purity, $diamond_color, $diamond_qual]);
...
return str_replace(' ','_', strtolower($value));
   ...
   $result = array();
  $color_value = $productcoll->getResource()->getAttribute('metal_color')->getFrontend()->getValue($productcoll);
...
foreach ($productAttributeOptions as $key => $value) {
    ...
    $productcoll1 =  $objectManager->create('Magento\Catalog\Model\Product')->load($product_id);
   //$productcoll1 = $this->_product->load($product_id);
  $result= false;      
  $productAttributeOptions = $this->_productTypeInstance->getConfigurableAttributesAsArray($productcoll1);
foreach ($productAttributeOptions as $key => $value) {
    $tmp_option = $value['values'];
     if($value['attribute_code'] == "metal_color") {
         $result = true;
     }
}
return $result;
